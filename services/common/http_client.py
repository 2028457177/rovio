...
封装 httpx 异步调用，带服务发现（通过 SERVICE_REGISTRY）。
...
class ServiceClient:
    ...
    用法::
        ...
        async with ServiceClient("auth") as client:
            resp = await client.get("/api/auth/me", token=user_token)
...
# 给 httpx.AsyncClient 加一个 request 别名（便于上面 _request 调用）
if not hasattr(httpx.AsyncClient, "request"):
    # httpx 本身有 request 方法，这里只是防御性注释
    pass
