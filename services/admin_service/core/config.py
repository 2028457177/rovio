"""admin_service 独立配置（纯环境变量驱动，无外部 YAML 依赖）。
...
# ===== 服务自身 =====
# 兼容旧变量名 ADMIN_PORT / ADMIN_DB（部署 .env 使用），优先新变量名 PORT / DB_NAME
PORT = int(os.getenv("PORT") or os.getenv("ADMIN_PORT") or "8005")
DB_NAME = os.getenv("DB_NAME") or os.getenv("ADMIN_DB") or "lc_admin"
...
def get_service_url(name: str) -> str:
    ...
    _targets = {
        ...
        or f"http://{_INTERNAL_HOST}:{os.getenv('USER_PORT', '8002')}",
        "chat": os.getenv("CHAT_SERVICE_URL")
        or f"http://{_INTERNAL_HOST}:{os.getenv('CHAT_PORT', '8003')}",
    }
    if name not in _targets:
        raise ValueError(f"admin_service 不支持调用服务: {name}")
