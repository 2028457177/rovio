"""auth_service 独立配置（纯环境变量驱动，无外部 YAML 依赖）。
...
# ===== 服务自身 =====
# 兼容旧变量名 AUTH_PORT / AUTH_DB（部署 .env 使用），优先新变量名 PORT / DB_NAME
PORT = int(os.getenv("PORT") or os.getenv("AUTH_PORT") or "8001")
DB_NAME = os.getenv("DB_NAME") or os.getenv("AUTH_DB") or "lc_auth"
...
def get_service_url(name: str) -> str:
    ...
    if name not in _targets:
        raise ValueError(f"auth_service 不支持调用服务: {name}")
