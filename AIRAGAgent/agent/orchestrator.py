...
## 知识库与联网的优先级（知识库优先，模型已有知识次之，联网极少兜底）
- 用户在问内部资料 / 操作指南 / 效率方法 / 文档处理技巧等内部知识 → 优先 knowledge（查知识库）
- 用户在问时效性信息（最新新闻、实时数据、今日热点、实时行情等）→ 可用 search（联网搜索），但要确认真的是非实时不可回答；能靠模型知识回答的，直接回答
- 系统提示"知识库已命中"时 → 优先 knowledge 或直接回答（subagent 留空），不要选 search 联网；
  仅当知识库内容明显不足以回答用户问题时才考虑 search
- 系统提示"知识库未命中"时 → **优先直接回答（subagent 留空），让模型基于已有知识回答**；
  只有问题明显依赖最新外部事实、且模型无法凭已有知识可靠回答时，才用 search 联网兜底
- 系统提示"联网搜索已关闭"时 → 任何情况都不要选 search，只靠知识库 / 已有知识回答
- 总结：search 是极少使用的兜底手段，默认不要让 search 参与计划；先 knowledge / 直接回答，想不明白再 search
...
## PBL 导学场景（《房屋建筑学》课程问题）
- 用户在问建筑学课程知识、住宅户型/卫生间/暗卫/平屋顶/楼梯/厨房等设计分析、构造做法类本科教学问题时，属于 **PBL 导学场景**。
- PBL 场景必须让 Orchestrator 直接回答（**subagent 留空字符串**），由 identity prompt 控制：先进行最多 3 轮苏格拉底式启发提问，学生完成思考后再输出完整知识点。
- **严禁一上来就派 knowledge 子代理检索知识库，或派 search 子代理联网搜索**；这些子代理会直接输出知识点/搜索结果，破坏三轮导学流程。
- 知识库命中时，相关素材会自动注入直接回答的上下文，仅供你组织启发问题与最终总结时作为后台专业素材，禁止直接复述成片知识库文本。
...
class Orchestrator:
    ...
    def _kb_route_hint(self, query: str, user_id: int) -> str:
        ...
        if search_enabled_var.get(True):
            return (
                ...
                "**优先直接回答（subagent 留空）**，让模型基于已有知识回答用户问题；\n"
                "只有问题明显依赖最新外部事实、且模型无法凭已有知识可靠回答时，才考虑用 search 联网搜索兜底。\n"
                "search 是极少使用的兜底手段，默认不要让 search 参与计划。\n"
            )
        return (
            "\n\n## 知识库提示\n"
    ...
    def execute_stream(self, query: str, chat_history: Optional[List[Dict[str, str]]] = None,
        ...
        if plan.status == PlanStatus.COMPLETED.value:
            ...
            # 点击展开即可随消息内联预览（可放大/下载）；文本里只留纯提示（不放下载链接，避免用户误点直接下载）
            ...
            if docs:
                ...
                hint = f"\n📄 已生成 {len(docs)} 个文档，点击下方文档卡片即可展开预览\n"
