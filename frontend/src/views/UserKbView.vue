    ...
    <header class="admin-header glass">
      <div class="header-left">
        ...
        <div class="title-block">
          <div class="title-row">
            <h1 class="header-title serif">我的知识库</h1>
            <span class="header-badge badge-user">个人</span>
          </div>
          <p class="header-sub">上传课件与笔记，AI 回答时优先参考你的资料</p>
        </div>
      </div>
      <div class="header-right">
        <span class="admin-name">{{ userName }}</span>
        <button class="btn-nav" @click="goChat" title="返回对话">
...
onMounted(() => {
  const user = getUser()
  userName.value = user?.display_name || user?.username || ''
})
...
.badge-user { background: transparent; color: var(--text-secondary); border: 1px solid var(--border); }
